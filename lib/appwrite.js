...
export const updateUserProfile = async (username, email) => {
  try {
    const user = await account.get(); // Obtém o usuário logado
    ...
    const existingUser = await databases.getDocument(
      config.databaseId,
      config.userCollectionId,
      userId // Certifique-se de que este ID foi usado ao criar o documento
    );
    if (!existingUser) {
    ...
    // Atualiza os dados do documento na coleção
    const userData = {
      username: username || user.name, // Atualiza o nome, se fornecido
      email: email || user.email, // Atualiza o e-mail, se fornecido
    };
    ...
    console.log("Dados atualizados na coleção:", databaseResponse);
